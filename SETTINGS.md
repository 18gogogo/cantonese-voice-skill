...
| 增益倍數 | 平均音量 | 最大音量 | 音量等級 |
|---------|---------|---------|---------|
| 原始 | -53.3 dB | -36.0 dB | 很小聲 |
| 4 倍 | -35.3 dB | -20.2 dB | 較小聲 |
| **6 倍（當前）** | **-25.8 dB** | **-11.2 dB** | **適中 ✅** |

**設定**:
```python
audio_gain = 6.0  # 增益倍数 6 倍
audio_data = audio_data * audio_gain
audio_data = np.clip(audio_data, -1.0, 1.0)  # 防止削波
```

**詳細記錄**: `/home/ubuntu/.openclaw/memory/VOLUME_GAIN_ADJUSTMENT_2026-02-11.md`
